<?php

namespace App\Controller;

use App\Entity\Curso;
use App\Entity\Tarea;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Annotation\Route;
use Symfony\Component\Security\Http\Attribute\IsGranted;

/**
 * Tareas para el profesor: ve solo los cursos que tiene asignados.
 *
 * El prefijo /profesor ya está cubierto por la regla ^/profesor de access_control, así que
 * security.yaml no se toca. El aislamiento por curso lo resuelve CursoVoter.
 *
 * @Route("/profesor/tareas")
 */
#[IsGranted('ROLE_PROFESOR')]
class TareaProfesorController extends AbstractTareaController
{
    protected function rutaBase(): string
    {
        return 'app_tarea_profesor';
    }

    /**
     * @return Curso[]
     */
    protected function cursosVisibles(): array
    {
        $usuario = $this->getUser();
        $profesor = $usuario->getProfesor();

        if (!$profesor) {
            return [];
        }

        // findByProfesor() no filtra por instituto, así que se filtra acá: el Voter igual
        // vuelve a validarlo por curso, pero el listado no debe mostrar cursos ajenos.
        $instituto = $usuario->getInstituto();

        return array_values(array_filter(
            $this->cursoRepository->findByProfesor($profesor),
            static fn($curso) => $curso->getInstituto() === $instituto
        ));
    }

    /**
     * @Route("/", name="app_tarea_profesor_index", methods={"GET"})
     */
    public function index(): Response
    {
        return $this->pantallaIndex();
    }

    /**
     * @Route("/curso/{id}", name="app_tarea_profesor_curso", methods={"GET"})
     */
    public function curso(Curso $curso): Response
    {
        return $this->pantallaCurso($curso);
    }

    /**
     * @Route("/curso/{id}/nueva", name="app_tarea_profesor_nueva", methods={"GET", "POST"})
     */
    public function nueva(Request $request, Curso $curso): Response
    {
        return $this->pantallaFormTarea($request, $curso, null);
    }

    /**
     * @Route("/{id}/editar", name="app_tarea_profesor_editar", methods={"GET", "POST"})
     */
    public function editar(Request $request, Tarea $tarea): Response
    {
        return $this->pantallaFormTarea($request, $tarea->getCurso(), $tarea);
    }

    /**
     * @Route("/{id}/eliminar", name="app_tarea_profesor_eliminar", methods={"POST"})
     */
    public function eliminar(Request $request, Tarea $tarea): Response
    {
        return $this->accionEliminarTarea($request, $tarea);
    }

    /**
     * @Route("/{id}", name="app_tarea_profesor_grilla", methods={"GET"})
     */
    public function grilla(Tarea $tarea): Response
    {
        return $this->pantallaGrilla($tarea);
    }

    /**
     * @Route("/{id}/guardar", name="app_tarea_profesor_guardar", methods={"POST"})
     */
    public function guardar(Request $request, Tarea $tarea): Response
    {
        return $this->accionGuardarGrilla($request, $tarea);
    }
}
